...
int main(int argc, char* argv[]){
  ...
  int BW_LIST[] = {6000}; //kbps
  int BW_NUM = sizeof(BW_LIST)/sizeof(BW_LIST[0]);
  int METHOD_LIST[] = {0, 1, 4, 20, 21};
  ...
  for(k=s_trace_id; k <= e_trace_id; k++){
	...
	for(bw_id = 0; bw_id < BW_NUM; bw_id++){
	  max_bw = BW_LIST[bw_id];
	  for(dec_id = 0; dec_id < vpParam.NO_SEG_FULL; dec_id++){
	    /* set througput */
	    adaptLogic->seg_thrp[dec_id] = max_bw;
	    if(vpParam.METHOD == 0){
...
void init(){
  ...
  for(frame_id = 0; frame_id < metadata->NO_SEG_FULL * vpParam.INTERVAL; frame_id ++){
    adaptLogic->est_vp_frame[frame_id][0] = v[(frame_id+1) * 2];
    adaptLogic->est_vp_frame[frame_id][1] = v[(frame_id+1) * 2 + 1];
   }
  //
  //  sprintf(buff, vpParam.head_trace);
  printf("#Load head trace: %s\n", vpParam.est_head_trace.c_str());
  sprintf(buff, "%s", vpParam.head_trace.c_str());
  metadata->import_matrix_from_txt_file(buff, v, rows_, cols_);
  for(frame_id = 0; frame_id < metadata->NO_SEG_FULL * vpParam.INTERVAL; frame_id ++){
    adaptLogic->frame_vp[frame_id][0] = v[(frame_id + 1) * 2];
...
void write_result(){
  ...
  int tid;
    // generate log files
  sprintf(fname, "result/video_%s/log_tile_ver_TRACE_%d_BW_%d_METHOD_%d_INTER_%d_BUFF_%d.txt",vpParam.videoname.c_str(), vpParam.TRACE, max_bw, vpParam.METHOD, vpParam.INTERVAL, vpParam.BUFFSIZE);
  printf("#%s\n", fname);
  log_tile_ver = fopen(fname, "w");
  sprintf(fname, "result/video_%s/log_tile_ver_linear_TRACE_%d_BW_%d_METHOD_%d_INTER_%d_BUFF_%d.txt",vpParam.videoname.c_str(),vpParam.TRACE,max_bw, vpParam.METHOD, vpParam.INTERVAL, vpParam.BUFFSIZE);
  log_tile_ver_linear = fopen(fname, "w");
  printf("#%s\n", fname);
  sprintf(fname, "result/video_%s/log_dec_TRACE_%d_BW_%d_METHOD_%d_INTER_%d_BUFF_%d.txt",vpParam.videoname.c_str(),vpParam.TRACE,max_bw, vpParam.METHOD, vpParam.INTERVAL, vpParam.BUFFSIZE);
  log_dec = fopen(fname, "w");
  sprintf(fname, "result/video_%s/log_frame_TRACE_%d_BW_%d_METHOD_%d_INTER_%d_BUFF_%d.txt",vpParam.videoname.c_str(), vpParam.TRACE, max_bw, vpParam.METHOD, vpParam.INTERVAL, vpParam.BUFFSIZE);
  printf("#%s\n", fname);
